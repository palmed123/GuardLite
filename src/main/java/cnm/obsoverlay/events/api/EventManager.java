package cnm.obsoverlay.events.api;

import cnm.obsoverlay.events.api.events.Event;
import cnm.obsoverlay.events.api.events.EventStoppable;
import cnm.obsoverlay.events.api.types.Priority;
...
public final class EventManager {
   ...
   public void unregister(Object object) {
      for (List<MethodData> dataList : this.REGISTRY_MAP.values()) {
   ...
   public void unregister(Object object, Class<? extends Event> eventClass) {
      if (this.REGISTRY_MAP.containsKey(eventClass)) {
         for (MethodData data : this.REGISTRY_MAP.get(eventClass)) {
            if (data.getSource().equals(object)) {
   ...
   public void removeEntry(Class<? extends Event> indexClass) {
      Iterator<Entry<Class<? extends Event>, List<MethodData>>> mapIterator = this.REGISTRY_MAP.entrySet().iterator();
   ...
   private void sortListValue(Class<? extends Event> indexClass) {
      ...
      for (byte priority : Priority.VALUE_ARRAY) {
         for (MethodData data : this.REGISTRY_MAP.get(indexClass)) {
